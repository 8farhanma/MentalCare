...
class MasterPenyakitController extends BaseController
{
    ...
    {
        $model = new PenyakitModel();
        $lastPenyakit = $model->orderBy('id_penyakit', 'DESC')->first();
        ...
        if ($lastPenyakit) {
            ...
            $newKodePenyakit = 'P' . str_pad($newNumber, 2, '0', STR_PAD_LEFT);
        }

        $data = [
            'title' => 'Tambah Penyakit',
            'newKodePenyakit' => $newKodePenyakit,
        ];
    ...
    public function delete($id_penyakit)
    {
        ...
        if ($countAturan > 0) {
            return redirect()->to('/admin/master_penyakit')->with('error', 'Penyakit tidak dapat dipindahkan ke recycle bin karena terkait dengan data aturan. Silahkan hapus aturan terlebih dahulu!');
        }

        // Lakukan soft delete
        $penyakitModel->delete($id_penyakit);

        return redirect()->to('/admin/master_penyakit')->with('success', 'Data penyakit berhasil dipindahkan ke Recycle Bin.');
    }
